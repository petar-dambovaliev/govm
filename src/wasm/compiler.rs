...
impl StackFrameInfo {
    fn find(&self, name: &str) -> Option<&StackLocal> {
...
impl GoType {
    ...
    fn from_type_name(name: &str) -> GoType {
        match name {
    ...
    fn from_val_type(vt: ValType) -> GoType {
        match vt {
...
impl LocalAlloc {
    ...
    fn find_go_type(&self, name: &str) -> Option<&GoType> {
        self.var_go_types.get(name)
    }

    fn set_go_type(&mut self, name: &str, gt: GoType) {
...
impl WasmCompiler {
    ...
    fn parse_go_int(s: &str) -> Result<i128, String> {
        let s = s.replace('_', "");
    ...
    fn is_integer_representable_float(s: &str) -> Option<i64> {
    ...
    fn forward_declare_functions(&mut self, decls: &[ast::Declaration]) {
        for decl in decls {
            let func_decl = match decl {
                ast::Declaration::Function(f) => f,
                _ => continue,
    ...
    fn is_unsigned_typed_expr(expr: &ast::Expression) -> bool {
    ...
    fn const_fits_type(v: i128, type_name: &str) -> bool {
    ...
    fn analyze_function_escapes(body: &ast::BlockStmt) -> HashSet<String> {
    ...
    fn mark_expr_escaping(expr: &ast::Expression, escaping: &mut HashSet<String>) {
        match expr {
            ast::Expression::Ident(id) => {
    ...
    fn escape_scan_call_args(expr: &ast::Expression, escaping: &mut HashSet<String>) {
        match expr {
            ast::Expression::Call(call) => {
                for arg in &call.args {
                    Self::mark_expr_escaping(arg, escaping);
    ...
    fn collect_stack_locals_from_block(
        stmts: &[ast::Statement],
        escaping: &HashSet<String>,
        struct_defs: &HashMap<String, StructDef>,
    ...
    fn collect_stack_locals_from_stmt(
        stmt: &ast::Statement,
        escaping: &HashSet<String>,
        struct_defs: &HashMap<String, StructDef>,
        locals: &mut Vec<StackLocal>,
    ...
    fn is_interface_field_selector(&self, expr: &ast::Expression, locals: &LocalAlloc) -> bool {
    ...
    fn is_call_returning_interface(&self, expr: &ast::Expression, locals: &LocalAlloc) -> bool {
    ...
    fn register_anon_interface(&mut self, iface: &ast::InterfaceType) -> String {
    ...
    fn resolve_type_assert_target(&mut self, target_type: &ast::Expression) -> Result<String, Error> {
    ...
    fn go_type_elem_size_and_align(elem_type: &ast::Expression) -> (i32, u32) {
        match elem_type {
            ast::Expression::Ident(id) => match id.name.as_str() {
    ...
    fn compile_composite_lit(
        ...
        for (i, kv) in comp.val.values.iter().enumerate() {
            ...
            if field_go_type_tag.as_deref() == Some("__interface") {
                if let ast::Expression::Ident(iface_ident) = elem_expr {
                    if iface_ident.name == "nil" {
                        ...
                        let concrete_type = locals.get_var_struct_type(&iface_ident.name)
                            .map(|s| s.to_string())
    ...
    fn coerce_go_types(from: &GoType, to: &GoType, out: &mut Vec<Instruction<'static>>) -> Result<(), Error> {
    ...
    fn find_method_func(&self, type_name: &str, method_name: &str) -> Option<&FuncInfo> {
    ...
    fn infer_type_alias_from_expr(&self, expr: &ast::Expression) -> Option<String> {
        match expr {
            ast::Expression::Selector(sel) => {
                if let ast::Expression::Ident(pkg) = sel.x.as_ref() {
                    let qualified = format!("{}.{}", pkg.name, sel.sel.name);
    ...
    fn count_go_level_returns(go_types: &[String]) -> usize {
        let mut count = 0;
        let mut i = 0;
